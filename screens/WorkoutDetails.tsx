  ...
  const handleSaveWebLink = async () => {
    ...
    try {
      await db.runAsync(
        ...
        [trimmedLink || null, editingMuscleGroup, exerciseNotesInput.trim(), editingSets?.toString() ?? 0, editingReps?.toString() ?? "0", editingExercise.exercise_id]
      );
              ...
              {day.exercises.length > 0 ? (
                day.exercises.map((exercise, index) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={index}
                      onPress={() => openWebLinkModal(exercise)} // Instead of the weblink Modal, an exercise update should be called
        ...
        <View style={[styles.modalContainer, { backgroundColor: 'rgba(0, 0, 0, 0.5)' }]}>
          <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
            ...
            <TextInput
              ...
              keyboardType="url"
            />
            <Text style={[styles.inputLabel, { color: theme.text, marginTop: 15 }]}>Sets</Text>
            <TextInput
              style={[styles.input, { color: theme.text, backgroundColor: theme.background, borderColor: theme.border }]}
              placeholder={t('setsPlaceholder')}
              placeholderTextColor={theme.text}
            ...
            />
            <Text style={[styles.inputLabel, { color: theme.text, marginTop: 15 }]}>Reps</Text>
            <TextInput
              style={[styles.input, { color: theme.text, backgroundColor: theme.background, borderColor: theme.border }]}
              placeholder={t('repsPlaceholder')}
              placeholderTextColor={theme.text}
